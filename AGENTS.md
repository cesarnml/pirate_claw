...
- `codex-preflight` requires invoking the `codex:rescue` skill first. Recording `clean` without running the skill is observable in the commit trail (no `[codexPreflight]` commit) and is a policy violation. `codex-preflight clean` requires a note summarizing what Codex reviewed and concluded — no note is a policy violation visible in the PR body.
