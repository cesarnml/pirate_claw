...
- `Implemented seam:` both ticket-linked and standalone review-detected branches now route through one shared processor that writes artifacts, runs triage, applies vendor fallback, persists thread resolutions when the mode/outcome allows it, and returns the normalized review snapshot consumed by the mode-specific callers.
- `Late review follow-up:` a later Greptile comment on PR `#63` pointed out that thread-resolution gating should follow the mapped `latestOutcome`, not the pre-mapped triage outcome. The follow-up patch keeps the current behavior unchanged for today's mappings while preventing future outcome remaps from resolving threads inconsistently.
