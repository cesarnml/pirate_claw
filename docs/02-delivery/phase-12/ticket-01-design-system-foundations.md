...
**Verification:** `bun run --cwd web test`, `check`, and `build` pass; full-repo `bun run verify` and `bun run test:web` pass from the repo root. Docker image build from `web/Dockerfile` was not re-run to completion in CI here (large base-layer pull); the production Vite build is the same step the Dockerfile uses after `bun install`.
