...
Phase 27 turns the Synology story from a hand-built Docker runbook into a DSM-first appliance install path. A DSM-first owner uses File Station, Docker GUI, or Container Manager — no SSH, no Docker CLI, no manual JSON edits.
...
- **SPK spike gates the installer shape.** P27.01 must produce a finding before the DSM 7.1 owner path is finalized. The spike failed, so the install path is a guided File Station + Docker GUI image import/mount flow — still fully GUI-only. Multiple Docker GUI image or volume mount steps are allowed when they are documented and do not require SSH, Docker CLI, or hand-edited files.
...
- DSM 7.1 Docker GUI release bundle
...
4. `P27.05 Install Health Daemon Endpoint` ← requires P27.02
5. `P27.06 First-Run Health UI` ← requires P27.05
6. `P27.07 Release Bundle Assembly` ← requires P27.03 + P27.06
7. `P27.08 Exit Validation and Screenshots` ← phase gate, requires P27.06 + P27.07
8. `P27.09 Onboarding UX Follow-up` ← requires P27.08; three UX gaps surfaced during validation
9. `P27.10 Docs and Phase Exit` ← docs-only, requires P27.09
...
- **Why:** P27 introduces a new install surface (DSM GUI release bundle, appliance path) that is unknown territory, creates a durable technical boundary, and will generate learning that shapes P28/P29 delivery assumptions.
