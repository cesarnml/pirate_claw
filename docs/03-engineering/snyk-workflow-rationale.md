...
- `Current scope:` the repo now carries explicit SonarQube Cloud support for standalone PR review orchestration, but only imports failed-check annotations into the `ai-review` flow. Lower-severity warning noise remains in SonarQube itself instead of entering the delivery triage loop by default. The GitHub Actions Snyk job is advisory, so credit exhaustion, timeouts, or Snyk findings remain visible in the job summary without failing the overall CI workflow.
